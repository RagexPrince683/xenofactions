...
public final class GuiWallArt extends GuiScreen {
  ...
  public void initGui() {
    ...
    urlField =
        new GuiTextField(fontRendererObj, left, top + 38, CONTROL_WIDTH, 20);
    ...
    buttonList.add(new GuiButton(BUTTON_HEIGHT, left + 125, top + 78, 115, 20,
                                 heightButtonText()));
    buttonList.add(new GuiButton(BUTTON_DOWNLOAD, this.width / 2 - 60,
                                 top + 108, 120, 20, "Download"));
  }
  ...
  protected void actionPerformed(GuiButton button) {
    if (button.id == BUTTON_WIDTH) {
      ...
      PacketDispatcher.wrapper.sendToServer(new WallArtConfigurePacket(
          dimension, controllerX, controllerY, controllerZ, displayWidth,
          displayHeight, enteredUrl));
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    int top = this.height / 2 - 70;
    drawCenteredString(fontRendererObj, "Wall Art", this.width / 2, top,
                       0xFFFFFF);
    drawString(fontRendererObj,
               "HTTPS Image URL:", this.width / 2 - CONTROL_WIDTH / 2, top + 25,
               0xA0A0A0);
    urlField.drawTextBox();
    drawCenteredString(fontRendererObj, "Display Size:", this.width / 2,
                       top + 66, 0xA0A0A0);
    if (statusMessage.length() > 0) {
      drawCenteredString(fontRendererObj, statusMessage, this.width / 2,
                         top + 133, 0xFF8080);
    }
  ...
  private String widthButtonText() { return "Width: " + displayWidth; }

  private String heightButtonText() { return "Height: " + displayHeight; }
}
