...
public class GUICityRelocationConfirm extends GuiScreen {
    ...
    @Override public void drawScreen(int mx,int my,float partial){drawDefaultBackground();drawCenteredString(fontRendererObj,"Move City Center: "+city,width/2,height/2-75,0xffffff);String[] lines={"The current City Center and territory remain active until the move succeeds.","Old chunks outside the new city radius become wilderness after success.","Moves farther than 10 horizontal blocks cost Prestige.","Each City Center can move 3 times within a rolling 24-hour window.","Canceling leaves the city unchanged."};int yy=height/2-48;for(String line:lines){drawCenteredString(fontRendererObj,line,width/2,yy,0xdddddd);yy+=14;}super.drawScreen(mx,my,partial);}
}
