...
public final class TDMBombManager {
    ...
    public static int getRemainingSeconds(World w){return state==BombRoundState.LIVE?(int)Math.max(0,(stateEndTick-w.getTotalWorldTime()+19)/20):0;}
    ...
    public static void startMatch(World world){
        ...
        cleanup(world,true); state=BombRoundState.PRE_ROUND; stateEndTick=world.getTotalWorldTime()+20;
    ...
    public static void tick(World world){
        ...
        if(state==BombRoundState.DISABLED){state=BombRoundState.PRE_ROUND;stateEndTick=now+20;}
        ...
        else if(state==BombRoundState.ROUND_END&&now>=stateEndTick) startRound(world);
    ...
    public static void startRound(World world){
        ...
        if(map==null||map.mode!=TDMManager.TDMGameMode.BOMB||!HbmCsgoChargeIntegration.isAvailable())return;
        if(!map.bombsiteA.isComplete()&&!map.bombsiteB.isComplete())return;
        ...
        if(red==0||blue==0){state=BombRoundState.PRE_ROUND;stateEndTick=world.getTotalWorldTime()+20;return;}
        eliminated.clear(); bomb=null; TDMSpectatorManager.restoreAll(); Random r=new Random();
        for(EntityPlayerMP p:TDMManager.getOnlinePlayers()) if(p.worldObj.provider.dimensionId==world.provider.dimensionId){TDMManager.respawnPlayer(p,r);TDMManager.promptForKit(p);}
