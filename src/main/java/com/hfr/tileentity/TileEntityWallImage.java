...
public class TileEntityWallImage extends TileEntity {
  ...
  public void initialize(UUID display, UUID owner, int face) {
    displayId = display;
    ownerId = owner;
  ...
  public void configure(int newWidth, int newHeight, String hash) {
    width = WallArtConstants.validSize(newWidth, newHeight) ? newWidth : 1;
    height = WallArtConstants.validSize(newWidth, newHeight) ? newHeight : 1;
    imageHash = WallArtConstants.validHash(hash) ? hash : "";
    markDirty();
  }
  ...
  public void readFromNBT(NBTTagCompound n) {
    super.readFromNBT(n);
    displayId = readUuid(n.getString("wallArtId"));
    ownerId = readUuid(n.getString("wallArtOwner"));
    facing = WallArtConstants.validFacing(n.getInteger("wallArtFacing"))
                 ? n.getInteger("wallArtFacing")
                 : 2;
    int w = n.getInteger("wallArtWidth"), h = n.getInteger("wallArtHeight");
    width = WallArtConstants.validSize(w, h) ? w : 1;
    height = WallArtConstants.validSize(w, h) ? h : 1;
    imageHash = WallArtConstants.validHash(n.getString("wallArtHash"))
                    ? n.getString("wallArtHash")
                    : "";
    requestGeneration = n.getLong("wallArtGeneration");
    // Legacy URL/image fields are deliberately ignored: clients must never
    // fetch them.
  }
  @Override
  public void writeToNBT(NBTTagCompound n) {
    ...
    n.setInteger("wallArtFacing", facing);
    n.setInteger("wallArtWidth", width);
    n.setInteger("wallArtHeight", height);
    n.setString("wallArtHash", imageHash == null ? "" : imageHash);
    n.setLong("wallArtGeneration", requestGeneration);
  }
  ...
  public AxisAlignedBB getRenderBoundingBox() {
    ...
    if (facing == 2)
      ...
      maxZ = zCoord + width;
    return AxisAlignedBB.getBoundingBox(minX, yCoord, minZ, maxX,
                                        yCoord + height, maxZ);
  }
  @Override
