...
public class BuilderDepotSnapshotPacket implements IMessage {
    ...
    public static BuilderDepotSnapshotPacket create(TileEntityMachineBuilder d){
        ...
        if(j!=null){s.state=j.state.name();s.statusKey=j.statusKey;s.detail=j.failureDetail;s.targetX=j.targetX;s.targetY=j.targetY;s.targetZ=j.targetZ;s.missing=j.missingQuantity;s.unsupported=j.unsupportedBlock.isEmpty()?0:1;Schematic sc=store==null?null:store.getSchematic(j.schematicId);s.schematic=sc==null?j.schematicId:sc.name;s.progress=sc==null?j.completedBlocks:satisfied(d,j,sc);s.total=sc==null?0:sc.size();s.bx=j.blockedX;s.by=j.blockedY;s.bz=j.blockedZ;s.blocked=j.state==BuilderState.INVALID_TERRITORY;}
    ...
    /** GUI snapshots are infrequent; count the world, not a deferrable scheduler cursor. */
    private static int satisfied(TileEntityMachineBuilder d,BuilderJob j,Schematic sc){int count=0,per=sc.width*sc.length;for(int i=0;i<sc.size();i++){int sy=i/per,r=i%per,sz=r%sc.length,sx=r/sc.length;net.minecraft.block.Block wanted=sc.resolveBlock(sx,sy,sz);if(wanted==null)continue;int meta=SchematicTransform.metadata(wanted,sc.getMetadata(sx,sy,sz),j.rotation,j.mirrored);int[] w=SchematicTransform.worldPosition(sc,sx,sy,sz,j.rotation,j.mirrored,j.originX,j.originY,j.originZ);if(d.getWorldObj().blockExists(w[0],w[1],w[2])&&d.getWorldObj().getBlock(w[0],w[1],w[2])==wanted&&d.getWorldObj().getBlockMetadata(w[0],w[1],w[2])==meta)count++;}j.completedBlocks=count;return count;}
