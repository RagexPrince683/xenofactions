...
Builders currently require **construction materials only**; they do not require pickaxes, axes, shovels, wrenches, or other tools to place or clear schematic cells. A Builder first works immediately when its current eye position has reach and line of sight; otherwise it navigates to a safe standing position within a four-block reach of its target rather than trying to stand inside the target. Candidate feet positions include cardinal and diagonal approaches above and below the target, exclude a solid placement target itself, and use collision geometry for body clearance and support so stairs and slabs remain valid walking surfaces. Vanilla navigation is enabled once through `EntityLiving`'s AI lifecycle, permits swimming and does not avoid ordinary water, allowing grass, dirt, slabs, shallow trenches, walls, ceilings, roofs, and upper floors without teleportation or flight. Lava remains hazardous.

Construction is scheduled deterministically from the lowest schematic Y layer upward, then by Z and X within a layer. Already matching cells are discovered from the world and skipped, including when a legacy active job is migrated to the versioned bottom-up work order. An unreachable cell is deferred while the Builder searches the rest of that layer; a placement restarts the layer pass so new floors and stairs immediately make deferred work eligible again. Only a complete layer pass with unresolved work and no usable reachable target enters `PATHFINDING_ERROR`.

For placement targets, the Builder resolves material needs before walking to the site. It carries materials in its persistent 27-slot inventory and transfers a bounded stack-sized batch from the Depot without duplicating stock. If the material is absent, the recoverable `WAITING_FOR_MATERIALS` state reports the exact item and cached remaining missing count, and automatically retries when stock appears. Items leave the carried inventory only after successful placement. Air targets never request materials; matching air advances immediately, while occupied air targets are cleared from work reach.

The active status describes the current operation and coordinate (moving and work positions, placing, or clearing). Placement and breaking failures distinguish invalid height, unsupported blocks, occupied targets, protected blocks, invalid territory, and rejected world edits. The same exact failure detail is persisted in the job, synchronized in Builder snapshots, and displayed in both Builder status screens. Progress is calculated from schematic cells actually satisfied in the world rather than the scheduler cursor; path attempts, successful clearing before replacement, and failed edits do not advance it.

## Canonical materials and Builder inventory

Every schematic block state is converted through Minecraft's block-to-item and `damageDropped` behavior before it becomes a requirement. Placement-only metadata such as stair facing, rail shape, door half, or switch orientation therefore never creates a fake item variant, while true inventory variants such as wool color, wood type, stone, slabs, and stained blocks remain distinct. The same registry item, canonical damage, and relevant NBT key is used for requirement aggregation, Depot counting/extraction, Builder carrying, and placement consumption.

Each Builder owns one **27-slot persistent inventory**. It merges normal stacks, is saved in `BuilderInventory` NBT (including migration of the former nine slot data), survives unloads and restarts, and is never cleared by pause, completion, or reassignment. The Builder NPC screen exposes these slots alongside the player's inventory and hotbar with normal shift-click transfer; the server resolves the Depot's assigned entity and revalidates faction build authority, dimension, assignment, and interaction distance.

While alive and working, a Builder periodically scans only a two-block neighborhood and picks up eligible dropped `EntityItem` stacks, including reusable drops produced while clearing construction cells. Partial pickup leaves overflow in the world. On death all carried stacks are dropped once through normal entity-item behavior.

Depot and Builder-carried quantities both count toward available construction materials on the Materials page. Fetching preflights Builder insertion capacity and transfers at most a useful stack-sized batch; if stock exists in the Depot but the Builder has no capacity, nothing is extracted and the distinct **Builder inventory is full** state is shown instead of a misleading missing-material message.
