...
Builders currently require **construction materials only**; they do not require pickaxes, axes, shovels, wrenches, or other tools to place or clear schematic cells. A Builder works from a safe standing position within a four-block reach of its target rather than trying to stand inside the target. Candidate positions include cardinal and diagonal approaches above, level with, and below the target, and must have collision-free feet/head space and solid footing. This allows ordinary walls, ceilings, roofs, and upper floors to be built from reachable surfaces without teleportation or flight.

Movement is watched for meaningful position or distance progress. A valid path is allowed to continue without being replaced every work cycle. When movement stalls, the Builder clears it and tries another safe standing position. After five unsuccessful path attempts, the persistent job enters `PATHFINDING_ERROR` and reports the target, last attempted work coordinate, reach, and attempt count. Retry/Resume clears that permanent error and performs a fresh reachable-position search. Structures that have no naturally reachable work position currently stop with this explicit error; Builders do not silently create scaffolding.
