...
BUY_PHASE kit selection and global buy protection are independent. The mandatory GUI is restored when displaced (without replacing `GuiGameOver`); selecting a valid kit closes it but retains the freeze and protection. A player completing Minecraft respawn during hardcore `PRE_ROUND` is enrolled into BUY_PHASE after teleport. Non-hardcore `RESPAWN_LOCK` remains a separate per-player restriction which ends immediately after that player selects a kit and never freezes other live players.
